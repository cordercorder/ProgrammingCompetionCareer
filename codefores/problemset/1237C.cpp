...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        ...
        p[i].z=lower_bound(Z.begin(),Z.end(),p[i].z)-Z.begin();
        rev[p[i].x].push_back(p[i]);
    }
    vector<node> remain,ret;
    for(int x=0;x<(int)X.size();x++){
        sort(rev[x].begin(),rev[x].end(),[](const node &a,const node &b)->bool{
            if(a.y<b.y){
                return true;
            }
        ...
        for(int i=0;i<(int)rev[x].size();i++){
            if(i+1<(int)rev[x].size()&&rev[x][i].y==rev[x][i+1].y){
                printf("%d %d\n",rev[x][i].id,rev[x][i+1].id);
                i++;
            }
            else{
                remain.push_back(rev[x][i]);
            }
        }
        for(int i=0;i+1<(int)remain.size();i+=2){
            printf("%d %d\n",remain[i].id,remain[i+1].id);
        }
        if((int)remain.size()%2==1){
            ret.push_back(remain.back());
        }
        remain.clear();
    }
