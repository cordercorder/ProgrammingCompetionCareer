...
int T;
mygeo::Point p[4],st,ed;
mygeo::Line line1,line2;
double xleft,ytop,xright,ybottom;

void handle(){
    p[0].x=min(xleft,xright);
    p[0].y=min(ytop,ybottom);
    p[1].x=max(xleft,xright);
    p[1].y=p[0].y;
    p[2].x=p[1].x;
    p[2].y=max(ytop,ybottom);
    p[3].x=p[0].x;
    p[3].y=p[2].y;
}

bool is_in(const mygeo::Point &po){
    if(mygeo::sign(po.x-p[0].x)>0&&mygeo::sign(p[1].x-po.x)>0
    &&mygeo::sign(po.y-p[0].y)>0&&mygeo::sign(p[3].y-po.y)>0
    )
        return true;
    return false;
}
...
int main(void){
    ...
    while(T--){
        f=false;
        scanf("%lf %lf %lf %lf %lf %lf %lf %lf",&st.x,&st.y,&ed.x,&ed.y,&xleft,&ytop,&xright,&ybottom);
        handle();
        line1=mygeo::Line(st,ed);
        for(int i=0;i<4;i++){
            line2=mygeo::Line(p[i],p[(i+1)%4]);
            if(line1.Len()>line2.Len()){
                tmp=line1.check_segpos_sim(line2);
            }
            else{
                tmp=line2.check_segpos_sim(line1);
            }
            if(tmp==0||tmp==3){
                f=true;
                break;
