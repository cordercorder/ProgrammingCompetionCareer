...
bool cmp(const mygeo::Point &a,const mygeo::Point b){
    int tmp=mygeo::sign((a-p[ret]).cross(b-p[ret]));
    if(tmp>0){
        return true;
    }
    else if(tmp==0&&mygeo::sign(p[ret].dis(a)-p[ret].dis(b))<0){
        return true;
    }
    return false;
}
...
int main(void){
    ...
    while(M--){
        scanf("%d",&N);
        for(int i=0;i<N;i++){
            scanf("%d %lf %lf",&id[i],&p[i].x,&p[i].y);
            if(mygeo::sign(p[i].y-p[0].y)<0){
                swap(p[i],p[0]);
                swap(id[0],id[i]);
            }
        }
