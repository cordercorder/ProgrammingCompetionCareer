...
int N;
Point p[maxn],rev[maxn];

bool cmp(const Point &a,const Point &b){
    int tmp=(a-p[1]).cross(b-p[1]);
    if(tmp>0){
        return true;
    }
    else if(tmp==0){
        return p[1].dis(a)-p[1].dis(b)<=0;
    }
    return false;
}
...
void solve(){
    ...
    for(int i=2;i<=N;i++){
        ...
        else if(p[i].y==p[id].y&&p[i].x<p[id].x){
            id=i;
        }
    }
    swap(p[id],p[1]);
    ...
    int tot=2;
    for(int i=3;i<=N;i++){
        while(tot>1&&(rev[tot]-rev[tot-1]).cross(p[i]-rev[tot-1])<0){
            tot--;
        }
        rev[++tot]=p[i];
    }
